...
i=0
while [ $i -lt $initial_processes ]; do
java -DlogFilename=logs/node$(($base_p2p_port + $i)) -cp target/asdProj2.jar Main -conf config.properties p2p_port=$(($base_p2p_port + $i)) server_port=$(($base_server_port + $i)) initial_membership=$membership 2>&1 | sed "s/^/[$(($base_p2p_port + $i))] /" &
...
if [ ! -z "$additional_processes" ] && [ $additional_processes -gt 0 ]; then
    ...
    while [ $j -lt $additional_processes ]; do
        ...
        java -DlogFilename=logs/node$(($base_p2p_port + $current_idx)) -cp target/asdProj2.jar Main -conf configLate.properties p2p_port=$(($base_p2p_port + $current_idx)) server_port=$(($base_server_port + $current_idx)) initial_membership=$membership 2>&1 | sed "s/^/[$(($base_p2p_port + $current_idx))] /" &
